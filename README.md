...
- **`@mantra/desktop`** — the Electron shell (P1 UI): fleet view + Decisions queue + a working
  **command console** (dual to voice) and a push-to-talk affordance. It is **set up entirely from the
  ...
  `~/.mantra/config.json` and applied live, so a double-clicked app can run without a shell env), a
  **GitHub token** (exported as `GH_TOKEN` so **Ship** works without a terminal `gh auth login`), adds/
  removes projects via a native **folder picker** (no hand-edited `projects.json`), and manages each
  project's **Ops monitors** (name + health URL, written to the repo's `.mantra/config.json`; the Ops
  loop rebuilds live). No file editing or CLI is needed to run the app end to end. The fleet view reflects
